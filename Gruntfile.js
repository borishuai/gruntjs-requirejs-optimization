module.exports = function(grunt){
    ...
    grunt.initConfig({
      ...
      useminPrepare: {
        html: ['build/*.html'],  //要处理的文件，我是将原始文件复制到build/index.html的，千万不要对原始文件直接作处理。
        options: {
      ...
      requirejs: {
        ...
        booklist: {
          options: {
            ...
            out: "build/static/booklist.js"
          }
        },
        bookDetail: {
          options: {
            include: ["common", "bookDetail"],
            out: "build/static/bookDetail.js"
          }
        }
      },
      preprocess: {
    ...
    // 默认任务
    grunt.registerTask('build', ['clean', 'copy:build', 'preprocess', 'requirejs', 'useminPrepare', 'concat', 'cssmin', 'filerev', 'usemin', 'cdn']);
